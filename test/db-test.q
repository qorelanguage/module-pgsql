...
class PgsqlTest inherits QUnit::Test {
   ...
   value varchar(160) not null)",
	...
	const Args = (
	    ...
	    "tinterval_f"     : pgsql_bind(PG_TYPE_TINTERVAL, "[\"May 10, 1947 23:59:12\" \"Jan 14, 1973 03:14:21\"]"),
	    "bytea_f"         : <bead>
	    #bit_f             :
	    #varbit_f          :
	    );
	...
	*string r = ndb.selectRow("select name from family where family_id = 3").name;
	...
	db.vexec("insert into data_test values (%v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v)", Args.values());
